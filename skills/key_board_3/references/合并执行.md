---
name: 合并执行
description: 合并多个 skill 为统一文档的 4 阶段执行手册（阅读真实代码/建主文档/归档原 skill/收尾验证）。当用户明确说"合并 skill / 把多个 skill 合成一个"时，经主 SKILL.md 流程判断确需合并后读取。探索式审计（未指定目标）走 [[合并审计]]。
---
...
>
> **使用条件**：当用户说"合并 skill"、"创建统一 skill"、"把多个 skill 合成一个"时，本 ref 是 key_board 主流程判断**确需合并后**的操作手册（未指定目标的探索式审计走 [[合并审计]]）。
>
...
注意：本 ref 中的"创建新 skill"指的是**合并产生的新 skill**，不是 key_board 的本职动作。未指定目标的探索式审计走 [[合并审计]]。
