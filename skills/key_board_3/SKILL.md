---
name: key_board_3
description: 当用户要求"拆分到references"、"给skill加ref引导"、"把xx沉淀为reference"、"优化skill结构"、"重构skill"、"skill膨胀了"、"合并skill"、或要求"探索/扫描现有skill看哪些可合并"时触发。本 skill 专用于按主题把已有 skill 组织成渐进式指导文档——主 SKILL.md 承担主干主题、references/ 承担特化专项的特化指导，整个 skill 包高内聚、单文档承担一个主题；长度只是边缘参考。不创建新 skill、不改变前置 skill (key_board / key_board_2) 的职责。探索模式详见 [[场景E-合并审计]]。
---
...
- "统一所有 skill 的 ref 架构"
- **"探索 / 扫描 / 观察现有 skill"、"哪些 skill 可合并"、"轻量 skill 是不是有重复"** → 走 [[场景E-合并审计]]（只出报告，不动手）
...
### 场景 E：合并候选审计（探索模式 ★ 前置发现）

**信号**：用户说"扫一遍现有 skill"、"哪些 skill 可合并"、"轻量 skill 是不是有重复"。

与 A-D 的本质区别：场景 A-D 是"**读 → 改一个 skill**"，场景 E 是"**读 N 个 skill → 出报告**"。

**E 不直接动手**：仅输出一份候选审计报告（含 4 维度评分、等级标准、合并方案）。报告交给用户决策后，再按对应条目触发 A/B/C/D 或合并场景。

**完整审计维度、等级标准、报告模板见 [[场景E-合并审计]]**。
...
**如触发场景 E**（用户说"扫所有 skill / 哪些可合并 / 看看有没有重复"）：

- 范围：`.claude/skills/` 下所有 skill 目录
- 目标：**只出审计报告**，不在 SOP 本步骤做任何修改
- 立即加载 [[场景E-合并审计]]，按其模板生成报告
- 用户选定条目后，再回到 Step 1-5 执行具体条目
...
**主题判断**：列完章节后,自问"这个 skill 是否真的承载了多个独立主题"。判断依据不是"内容多少",而是"读这些章节的人,会不会因为场景不同而只读其中一部分"。是 → 拆;否 → 不拆。

---

### Step 3: 设计方案

- **场景 C/A**：ref 名称、文件路径、加载时机、预估拆分原因(为什么这是个独立主题)
- **场景 B**：只在末尾加引导段
- **场景 D**：先选模板 skill
