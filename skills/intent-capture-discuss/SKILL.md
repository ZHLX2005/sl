...
### ▸ LOW V1 原则（主动落文档的最低门槛）

调用本 skill 的默认动作就是沉淀文档，**v1 准入门槛刻意放低**：

1. **模糊输入也算信号**——"看一下 / 总结一下 / 梳理一下 / 对比一下 / 明确一下 / 学习一下" 等不带明确产物名的输入，Phase 1 也要判别模式 + 起一个 topic-slug，先落一份 v1 现状文档（项目学习模式）或首版基础概念（基础知识模式），后续轮次再补完。
2. **小知识点也是文档**——讨论中浮出的单点机制笔记、术语解释、命令记录、配置备忘、踩坑总结，**默认要作为首版文档的一部分一起沉淀**，不必等话题完整闭环再生成。可走 `basic/` 2e 文档或随主题文档并入，不另起孤立的小笔记。
3. **v1 不必求全**——版本不覆盖、旧版只读（铁律 3），所以 v1 不必包含全部维度、不必等到讨论结束才生成，先固化当前认知，后续 Phase 4 自然产出 v2/v3。
...
按上述「LOW V1 原则」，Phase 1 完成后不要求用户再次发信号就要落文档：

* **模式已判定 + 主题已定** → 默认推进到 Phase 2 多轮讨论，过程中凡出现可固化的认知（结构盘点、机制理解、术语定义、对比结论、踩坑），**默认攒入待落文档列表**。
* **多轮中出现「我明白了」「原来是 X」「机制是 Y」等确认性表达** → 默认触发"现在是不是要落一下"的微确认或直接落基础版，无需等用户说"生成文档"。
* **整轮讨论出现明显闭环信号**（用户说"差不多 / 先到这 / 总结一下"等）→ 默认推进到 Phase 3 生成对应类型文档，不要求再二次确认"要不要生成"。
* **首版 v1 不必完整**——后续 Phase 4 自然产出 v2/v3 迭代。
