---
name: merge-skill
description: Use when需要合并多个已存在的skill到新的统一skill中，并保持文档与真实代码同步。触发条件：用户要求合并skill、创建统一skill文档、创建skill的references目录。
---
# Merge Skill — 合并多个 Skill 为统一文档

**REQUIRED SUB-SKILL:** 使用 `superpowers:writing-skills `或者/skill-creator  作为底层流程指导。

如果无法加载 请立刻停止 并且反馈给用户
