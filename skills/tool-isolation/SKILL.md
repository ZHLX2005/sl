...
```
TOOL\_NAME="http-tester"   # 或 "esbuild-checker"

mkdir -p ".tool/\${TOOL\_NAME}/scripts"
...
AUTH\_TOKEN = "your-secret-token-change-me"  # 与 .env 对齐

def build\_headers(token: str | None = None) -> dict\[str, str]:

&#x20;   """构造带 JWT 头的请求头。"""

&#x20;   payload = {"sub": "tester", "role": "admin"}

&#x20;   jwt\_token = jwt.encode(payload, AUTH\_TOKEN, algorithm="HS256")
...
&#x20;   headers = build\_headers()

&#x20;   with httpx.Client(base\_url=BASE\_URL, headers=headers, timeout=10.0) as client:
...
&#x20;           "elapsed\_ms": int(resp.elapsed.total\_seconds() \* 1000),
...
&#x20;   parser.add\_argument("path", help="API 路径，如 /api/v1/kv/list")

&#x20;   parser.add\_argument("--method", "-X", default="GET")

&#x20;   parser.add\_argument("--body", "-d", type=json.loads, default=None)
...
npm install --save-dev esbuild       # 依赖装到本工具的 node\_modules
```
