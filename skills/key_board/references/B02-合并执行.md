---
name: 合并执行
description: 合并多个 skill 为统一文档的 4 阶段执行手册（阅读真实代码/建主文档/归档原 skill/收尾验证）。当用户明确说"合并 skill / 把多个 skill 合成一个"时，经主 SKILL.md 流程判断确需合并后读取。探索式审计（未指定目标）走 [[B01-合并审计]]。
---

# 合并执行 — 4 阶段手册

> 本文档来自原 `merge-skill/SKILL.md`，作为 key_board 在「**合并多个 skill**」这种**特殊优化场景**下的执行手册。
>
> **使用条件**：当用户说"合并 skill"、"创建统一 skill"、"把多个 skill 合成一个"时，本 ref 是 key_board 主流程判断**确需合并后**的操作手册（未指定目标的探索式审计走 [[B01-合并审计]]）。
>
> key_board 主流程仍负责结构判断（是否真要合并、拆出去不行吗），本 ref 负责**动手合并时的 4 阶段流程细节**。
...
| 文件 | 内容 |
|------|------|
| `references/<原skill1>.md` | <原 skill1 的核心内容描述> |
| `references/<原skill2>.md` | <原 skill2 的核心内容描述> |

**说明**：主文档基于真实代码重新编写，合并了各原始 skill 的核心概念，并更正了与实际代码的差异。原始文档保留作为历史参考。
```
...
```
Step 1.1: 确定代码文件
  → Glob {pattern: "**/*<主题>*"} 搜索相关代码
  → Grep <入口关键词> manifest.json 确认入口

Step 1.2: 读取代码
  → 按架构层级逐层 Read 相关实现文件

Step 1.3: 对比 skill 文档
  → Read 各原 skill 的 SKILL.md
  → 逐函数对比：实现 vs 文档
...
Step 3.2: 写入归档
  → Write references/<原skillN>.md (加 reference frontmatter)
...
```
Step 4.1: 验证归档
  → Read references/<原skillN>.md 确认完整

Step 4.2: 删除
  → Bash: rm -rf .claude/skills/<原skillN>

Step 4.3: 验证
  → Glob .claude/skills/<原skillN> 确认不存在
...
key_board 主 SKILL.md 负责**结构判断**（是否要合并、拆出去不行吗），本 ref 负责**动手合并时的执行细节**。

调用顺序：

1. 先走 key_board 主流程 → 判断"合并"确有必要
2. 加载本 ref → 获得 4 阶段合并流程
3. 合并过程中**仍要遵循 key_board 的核心原则**：不主动创建 skill（这次创建新 skill 是合并的副产品）、不修改前置 skill、按主题自然分配内容、不为主文档减肥而硬拆

注意：本 ref 中的"创建新 skill"指的是**合并产生的新 skill**，不是 key_board 的本职动作。未指定目标的探索式审计走 [[B01-合并审计]]。
